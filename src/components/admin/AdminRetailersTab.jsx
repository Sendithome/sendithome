...
import { CheckCircle2, X, Loader2, Search, ExternalLink, Download } from 'lucide-react';
...
export default function AdminRetailersTab({ retailers, onRefresh }) {
  ...
  const handleReject = async (retailer) => {
    if (!window.confirm(`Reject "${retailer.store_name}"? They will not be able to access the retailer portal.`)) return;
    setProcessing(retailer.id);
  ...
  const handleExport = () => {
    const rows = [['Store', 'Brand', 'Contact', 'Email', 'Category', 'Location', 'Partner Code', 'Commission %', 'Status']];
    filtered.forEach(r => rows.push([
      r.store_name || '', r.brand_name || '', r.contact_name || '', r.contact_email || '',
      r.store_category || '', r.store_location || '', r.partner_code || '', r.commission_rate ?? '', r.status || '',
    ]));
